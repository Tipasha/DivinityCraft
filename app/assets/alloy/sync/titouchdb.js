...
var _ = require('alloy/underscore'), titouchdb = require('com.obscure.titouchdb'), manager = titouchdb.databaseManager, db;
...
function query_view(db, name, options) {
	...
	if (_.isBoolean(opts.prefetch)) {
		query.prefetch = opts.prefetch;
	}
	if (_.isFinite(opts.limit)) {
		query.limit = opts.limit;
	}
	if (_.isFinite(opts.skip)) {
		query.skip = opts.skip;
	}
	if (_.isBoolean(opts.descending)) {
		query.descending = opts.descending;
	}
	if (_.isBoolean(opts.includeDeleted)) {
		query.includeDeleted = opts.includeDeleted;
	}
	if (_.isFinite(opts.groupLevel)) {
		query.groupLevel = opts.groupLevel;
	}
	if (_.isFinite(opts.indexUpdateMode)) {
		query.indexUpdateMode = opts.indexUpdateMode;
	}
	if (_.isBoolean(opts.mapOnly)) {
		query.mapOnly = opts.mapOnly;
	}

	if (opts.startKey) {
		query.startKey = opts.startKey;
	}
	if (opts.endKey) {
		query.endKey = opts.endKey;
	}

	if (opts.startKeyDocID) {
		query.startKeyDocID = opts.startKeyDocID;
	}
	if (opts.endKeyDocID) {
		query.endKeyDocID = opts.endKeyDocID;
	}

	if (_.isArray(opts.keys)) {
...
function Sync(method, model, options) {
	...
	case 'create':
		...
		if (!err) {
			...
			resp = model.toJSON();
			!opts.silent && model.trigger('change', {
				fromAdapter : true
			});
		}
	...
	case 'read':
		if (model.idAttribute) {
			...
			!opts.silent && collection.trigger('fetch', {
				fromAdapter : true
			});
		}
		break;
	...
	case 'delete':
		if (model.id) {
			...
			if (!err) {
				resp = model.toJSON();
				!opts.silent && model.trigger('destroy', {
					fromAdapter : true
				});
			}
		}
...
module.exports.afterModelCreate = function(Model) {
	...
	// true for all TouchDB documents
