...
XHR.prototype.get = function(url, onSuccess, onError, extraParams) {
	...
	//extraParams.timeout = extraParams.timeout || 5000;
	...
	if (!extraParams.ttl || cache == 0) {
		...
		xhr.onload = function() {
			...
			if (xhr.getResponseHeader('Set-Cookie') && xhr.getResponseHeader('Set-Cookie').length > 0) {
				var _b = xhr.getResponseHeader('Set-Cookie').match(/_b=\d+(?=;)/g);
				if (_b && _b.length > 0) {
					result.header = _b[0].substr(3, _b[0].length - 3);
				}
			}
